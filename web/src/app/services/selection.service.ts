...
export class SelectionService {
  ...
  public toggleSection(section: Section) {
    if (!this.selectedTermService.isCurrentTermActive) {
      return;
    }
  ...
  public removeSection(section: Section) {
    ...
    let store = this.getSelections() || {};
    if (
      !store[section.listing.id] ||
      !store[section.listing.id].includes(section.id)
    )
      return false;
    store[section.listing.id].splice(
      store[section.listing.id].indexOf(section.id),
      1
    );
