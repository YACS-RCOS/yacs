...
export class ScheduleViewComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public ngAfterViewInit(): void {
    this.ScheduleList.changes.subscribe(
      (comps: QueryList<ScheduleComponent>) => {
  ...
  private getSchedules(): void {
    ...
    Schedule.where({ section_id: sectionIds })
      .includes('sections')
  ...
  public get activeScheduleIndex(): number {
    return this.scheduleSet.numSchedules > 0
      ? this.scheduleSet.activeScheduleIndex + 1
      : 0;
  }

  public get statusText(): string {
    if (this.scheduleSet.activeSections.length > 0) {
      return `CRNs: ${this.scheduleSet.activeSections
        .map(s => s.crn)
        .join(', ')}`;
    }
  ...
  public downloadImage(): void {
    ...
    domtoimage
      .toPng(node, { bgcolor: 'white', quality: 1.0 })
