...
export class ScheduleComponent implements AfterViewInit {
  ...
  public get days(): Day[] {
    const days = [];
    for (
      let day = this.scheduleSet.startDay;
      day <= this.scheduleSet.endDay;
      ++day
    ) {
  ...
  public get hours(): string[] {
    const hours = [];
    for (
      let time = this.scheduleSet.startTime;
      time < this.scheduleSet.endTime;
      time += 60
    ) {
  ...
  public eventPosition(period: Period): number {
    ...
    return (
      this.scheduleSet.height *
      ((eventStart - this.scheduleSet.startTime) / this.scheduleSet.numMinutes)
    );
  }

  public eventHeight(period: Period): number {
    const eventDuration =
      this.toMinutes(period.end) - this.toMinutes(period.start);
    return (
      this.scheduleSet.height * (eventDuration / this.scheduleSet.numMinutes)
    );
  }
