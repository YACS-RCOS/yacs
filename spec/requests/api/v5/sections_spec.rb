def match_sections(sections, course)
  sections.each_with_index do |section, n|
    ...
    expect(json['sections'][n]['crn'])         .to eq section.crn
    expect(json['sections'][n]['name'])        .to eq section.name
    expect(json['sections'][n]['seats'])       .to eq section.seats
    expect(json['sections'][n]['seats_taken']) .to eq section.seats_taken
    expect(json['sections'][n]['course_id'])   .to eq course.id
    section.periods.each_with_index do |period, n|
      expect(json['sections'][n]['period']['id']).to eq period.id
    end
  end
end
...
describe 'Sections API' do
  context 'when there is a course' do
    ...
    it '#index' do
      ...
      expect(json['sections'].length) .to eq 10
